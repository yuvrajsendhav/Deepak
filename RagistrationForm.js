...
export default function RegistrationForm(){
  ...
  const handleValue = ()=>{
      ...
      axios.post('http://localhost:5000/feed/postdata',{name, mobNo, email, password, password2, city})
      .then(res=>window.location='/LoginF',
          ...
          <div className="container">
            ...
            <form style={{ width:'100%', margin:'0% 40% '}}>
              ...
              <div className="form-group">
                ...
                <small id="citydErr" className="form-text text-muted">{cityErr}</small>
              </div>
              <button type="submit" className="btn btn-primary " onClick={handleValue}><Link to="/LoginF" className=" text-dark">Submit</Link></button>
              <button type="reset" className="btn btn-secondary " style={{marginLeft:'1%'}}>Reset</button>
            {name} {mobNo} {email}  {password}  {password2} {city}
            </form>
        </div>
    );
}